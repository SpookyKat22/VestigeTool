...
namespace _4RTools.Forms
{
    public partial class SkillTimerForm : Form, IObserver
    {
        ...
        private string NormalizeDelayForDisplay(int refreshDelay)
        {
            return (refreshDelay > 0 && refreshDelay < 100 ? refreshDelay * 1000 : refreshDelay).ToString();
        }
        ...
        public void Update(ISubject subject)
        {
            switch ((subject as Subject).Message.code)
            {
                case MessageCode.PROFILE_CHANGED:
                    ...
                    string autoRefreshDelay1 = NormalizeDelayForDisplay(ProfileSingleton.GetCurrent().AutoRefreshSpammer1.RefreshDelay);

                    string skillTimerKey2 = ProfileSingleton.GetCurrent().AutoRefreshSpammer2.RefreshKey.ToString();
                    string autoRefreshDelay2 = NormalizeDelayForDisplay(ProfileSingleton.GetCurrent().AutoRefreshSpammer2.RefreshDelay);

                    string skillTimerKey3 = ProfileSingleton.GetCurrent().AutoRefreshSpammer3.RefreshKey.ToString();
                    string autoRefreshDelay3 = NormalizeDelayForDisplay(ProfileSingleton.GetCurrent().AutoRefreshSpammer3.RefreshDelay);
        ...
        private void txthDelay1TextChanged(object sender, EventArgs e)
        {
            try
            {
                ProfileSingleton.GetCurrent().AutoRefreshSpammer1.RefreshDelay = Convert.ToInt32(this.txtAutoRefreshDelay.Value);
                ProfileSingleton.SetConfiguration(ProfileSingleton.GetCurrent().AutoRefreshSpammer1);
            }
            catch(Exception ex) {
        ...
        private void txthDelay2TextChanged(object sender, EventArgs e)
        {
            try
            {
                ProfileSingleton.GetCurrent().AutoRefreshSpammer2.RefreshDelay = Convert.ToInt32(this.txtAutoRefreshDelay2.Value);
                ProfileSingleton.SetConfiguration(ProfileSingleton.GetCurrent().AutoRefreshSpammer2);
            }
            catch (Exception ex)
            {
        ...
        private void txthDelay3TextChanged(object sender, EventArgs e)
        {
            try
            {
                ProfileSingleton.GetCurrent().AutoRefreshSpammer3.RefreshDelay = Convert.ToInt32(this.txtAutoRefreshDelay3.Value);
                ProfileSingleton.SetConfiguration(ProfileSingleton.GetCurrent().AutoRefreshSpammer3);
            }
            catch (Exception ex)
            {
